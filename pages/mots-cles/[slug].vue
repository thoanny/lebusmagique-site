...
if (!data.value?.tag) {
    ...
    pagination.value = {
        hasNextPage: data.value.tag.posts.pageInfo.hasNextPage,
        endCursor: data.value.tag.posts.pageInfo.endCursor,
    };
}
...
<template>
    <!-- TODO : SEO -->
    <h1 class="text-4xl font-bold mb-6">Mot-clé&nbsp;: {{ data.tag.name }}</h1>
